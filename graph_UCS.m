function [path,frontierList,exploredList] = graph_UCS(distanceMatrix, placeCoords, placeNames, startNode, goalNode)
    ...
    g(startNode) = 0; % distance travel to each node 
    ...
    while length(frontierList) > 0  % while the frontier is not empty
        ...
        for n = neighbours(distanceMatrix, expandingNode)  % for each neighbour % record the distance travelled
            ...
            if ~qcontains(frontierList, n) && ~qcontains(exploredList, n)
                ...
                frontierList = qinsert(frontierList,n,g);
